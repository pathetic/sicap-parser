...
const retryOn = (caNoticeId, method, day) => (attempt, error, response) => {
  if (!response) {
    fs.appendFileSync(
      "log-retries.log",
      `${day} > ${caNoticeId}|${method}|no response|${attempt} ... ERR: ${error} \n`
    )
    return true
  }
...
export async function getAll(date, { istoric = false }) {
  ...
  const response = await fetch(`https://${istoricUrlFragment}e-licitatie.ro/api-pub/NoticeCommon/GetCANoticeList/`, {
    retries,
    retryDelay,
    method: "POST",
    headers: {
...
export async function getPublicNotice(caNoticeId, { day, istoric = false }) {
  ...
  try {
    const response = await fetch(
      `https://${istoricUrlFragment}e-licitatie.ro/api-pub/C_PUBLIC_CANotice/get/${caNoticeId}`,
      {
        retries,
        retryDelay,
...
export async function getPublicNoticeContracts(caNoticeId, { day, istoric = false }) {
  ...
  const response = await fetch(
    `https://${istoricUrlFragment}e-licitatie.ro/api-pub/C_PUBLIC_CANotice/GetCANoticeContracts`,
    {
      retries,
      retryDelay,
...
export async function getAllDirect(
  ...
  const response = await fetch(
    `https://${istoricUrlFragment}e-licitatie.ro/api-pub/DirectAcquisitionCommon/GetDirectAcquisitionList/`,
    {
      retries,
      retryDelay,
      method: "POST",
...
export async function getPublicDirectAcquisition(id, { istoric = false }) {
  ...
  const response = await fetch(
    `https://${istoricUrlFragment}e-licitatie.ro/api-pub/PublicDirectAcquisition/getView/${id}`,
    {
      retries,
      retryDelay,
...
export async function getCAEntityView(entityId, { istoric = false }) {
  ...
  const response = await fetch(
    `https://${istoricUrlFragment}e-licitatie.ro/api-pub/Entity/getCAEntityView/${entityId}`,
    {
      retries,
      retryDelay,
...
export async function getSUEntityView(entityId, { istoric = false }) {
  ...
  const response = await fetch(
    `https://${istoricUrlFragment}e-licitatie.ro/api-pub/Entity/getSUEntityView/${entityId}`,
    {
      retries,
      retryDelay,
...
export async function getAllDaAwardNotices(
  date,
  {
    endDate = null,
    pageIndex = 0,
  ...
  const body = {
    ...
    publicationDateEnd: endDate || date,
  }

  const response = await fetch(
    `https://${istoricUrlFragment}e-licitatie.ro/api-pub/DaAwardNoticeCommon/GetDaAwardNoticeList/`,
    {
      retries,
      retryDelay,
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
        Referer: "https://e-licitatie.ro/pub/da-award-notices/list/0",
      },
      body: JSON.stringify(body),
    }
  )
...
export async function getPublicDaAwardNotice(id, { istoric = false }) {
  const istoricUrlFragment = istoric ? "istoric." : ""

  const response = await fetch(
    `https://${istoricUrlFragment}e-licitatie.ro/api-pub/PublicDaAwardNotice/getView/${id}`,
    {
      retries,
      retryDelay,
      retryOn: retryOn(id, "getPublicDaAwardNotice"),
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
        Referer: "https://e-licitatie.ro/pub/da-award-notices/list/0",
      },
...
export async function searchSuppliers(cui, { istoric = false }) {
  ...
  const response = await fetch(
    `https://${istoricUrlFragment}e-licitatie.ro/api-pub/ComboPub/searchSuppliers?filter=${cui}&pageIndex=0&pageSize=20&parentId=`,
    {
      retries,
      retryDelay,
...
export async function searchCpvs(pageIndex = 0) {
  const response = await fetch(
    `https://e-licitatie.ro/api-pub/ComboPub/searchCpvs?filter=&pageIndex=${pageIndex}&pageSize=100&parentId=`,
    {
      retries,
      retryDelay,
...
export async function getAuthority(id, { client, istoric }) {
  let authority = null
  ...
  if (!result || result.body.hits.hits.length === 0) {
    authority = await getCAEntityView(id, { istoric })
    await client.update({
      id,
      index: "autoritati",
      body: {
    ...
    authority = { ...result.body.hits.hits[0]._source.data }
  }

  return authority
}

export async function getSupplier(id, { client, istoric }) {
  let supplier = null
  ...
  if (!result || result.body.hits.hits.length === 0) {
    ...
    supplier = await getSUEntityView(id, { istoric })
    // save supplier
    await client.update({
      id,
      index: "firme",
    ...
    supplier = { ...result.body.hits.hits[0]._source.data }
  }
